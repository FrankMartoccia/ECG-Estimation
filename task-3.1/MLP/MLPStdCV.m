...
numFolds = 5;
cv = cvpartition(length(InputStdECG), 'KFold', numFolds);
...
for i = 1:length(trainFunctions)
    ...
    for fold = 1:numFolds
        ...
        trainData = InputStdECG(:, trainIdx);
        trainTarget = TargetStdECG(:, trainIdx);
        testData = InputStdECG(:, testIdx);
        testTarget = TargetStdECG(:, testIdx);
